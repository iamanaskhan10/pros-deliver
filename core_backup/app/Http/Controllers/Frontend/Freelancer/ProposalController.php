...
class ProposalController extends Controller
{
    public function all_proposal()
    {
        ...
        return view('frontend.user.influencer.proposal.proposals',compact(['all_proposals','jobs']));
    }
    ...
    function pagination(Request $request)
    {
        if($request->ajax()){
            $all_proposals = JobProposal::with('job')
                ...
                ->latest()
                ->paginate(10);
            return view('frontend.user.influencer.proposal.search-result', compact(['all_proposals']))->render();
        }
    }
}
