<div class="single-profile-settings d-flex flex-wrap justify-content-between border-all"
     id="display_freelancer_profile_photo">
    <div class="single-profile-settings-flex">
        <div class="single-profile-settings-thumb">
            @php
                use Illuminate\Support\Facades\Auth;$user = Auth::guard('web')->user();
        ...
        <div class="single-profile-settings-contents">
            ...
            <p class="single-profile-settings-contents-para mt-2">
                {{ __('Profile photo recomended size 300x300 pixels') }}
            </p>
        </div>
    </div>
    <div class="btn-wrapper">
        @php $feedback = \App\Models\Feedback::where('user_id',Auth::guard('web')->user()->id)->first() @endphp
        <a href="javascript:void(0)" class="btn-profile btn-outline-1 open_freelancer_feedback_modal"
