                    ...
                    if(category == '' || subcategory == '' || title == '' || description == '' || level == '' || duration==''){
                        ...
                        @if(moduleExists('SecurityManage'))
                            @if(Auth::guard('web')->user()->freeze_job == 'freeze')
                                $('.setup-footer-right').html('<a href="#" class="btn-profile btn-bg-1 @if(Auth::guard('web')->user()->freeze_job == 'freeze') disabled-link @endif">{{ __('Update Campaign') }}</a>');
                            @else
                                $('.setup-footer-right').html('<button type="submit" class="btn-profile btn-bg-1" id="confirm_edit_job">{{ __('Update Campaign') }}<span id="job_edit_load_spinner"></span></button>');
                            @endif
                        @else
                            $('.setup-footer-right').html('<button type="submit" class="btn-profile btn-bg-1" id="confirm_edit_job">{{ __('Update Campaign') }}<span id="job_edit_load_spinner"></span></button>');
                        @endif
