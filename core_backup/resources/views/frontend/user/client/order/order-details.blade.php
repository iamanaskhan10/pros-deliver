                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                                        ...
                                        <div class="myOrder-single-content">
                                            <div class="d-flex gap-2 justify-content-between">
                                                ...
                                                <span class="myOrder-single-content-time inf-tag"><i
                                                    ...
                                                    {{ $order_details->created_at->diffForHumans() }} </span>
                                            </div>
                                ...
                                <div class="myOrder-single-item">
                                    <div class="myOrder-single-flex flex-between">
                                        ...
                                        <div class="btn-wrapper flex-btn">
                                            @if ($order_details?->user?->is_suspend != 1)
                                                ...
                                                @if ($order_details->status == 3)
                                                    <a href="{{ route('client.order.invoice.generate', $order_details->id) }}"
                                        ...
                                        @if ($order_details->is_fixed_hourly == 'hourly' && $order_details->status != 0)
                                            <a href="{{ route('client.order.work.history', $order_details->id) }}"
                                        ...
                                        <div class="myJob-wrapper-single-balance">
                                            <div class="myJob-wrapper-single-balance-contents">
                                                <div
                                                    ...
                                                    @else
                                                        @php
                                                            $earnings = \App\Models\OrderMilestone::where(
                                                                'order_id',
                                                                $order_details->id,
                                                            )
                                                                ->where('status', 2)
                                                    ...
                                                    <span class="myJob-wrapper-single-balance-icon hover-question">
                                                        ...
                                                        <span
                                                            class="hover-active-content">{{ __('Earned balance means how much amount influencer have received for this order.') }}</span>
                                                    </span>
                                                </div>
                                                ...
                                                <div class="myJob-wrapper-single-balance-contents">
                                                    <div
                                                        ...
                                                        <span class="myJob-wrapper-single-balance-icon hover-question">
                                                            <i class="fa-solid fa-question"></i>
                                                            <span
                                                                class="hover-active-content">{{ __('Hourly rate means how much amount client will pay for each hour after complete the order.') }}</span>
                                                        </span>
                                                    </div>
                                                    <p class="myJob-wrapper-single-balance-para">{{ __('Hourly Rate') }}
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="myJob-wrapper-single-balance">
                                                <div class="myJob-wrapper-single-balance-contents">
                                                    <div
                                                        class="myJob-wrapper-single-balance-price d-flex gap-2 justify-content-between">
                                                        <span
                                                            class="price-title">{{ $order_details?->job->estimated_hours }}</span>
                                                        <span class="myJob-wrapper-single-balance-icon hover-question">
                                                            <i class="fa-solid fa-question"></i>
                                                            <span
                                                                class="hover-active-content">{{ __('Estimated hours refer to the approximate time a client can set for completing the order. The client can adjust this time before accepting the order.') }}</span>
                                                        </span>
                                                    </div>
                                                    <p class="myJob-wrapper-single-balance-para">
                                                        {{ __('Estimated Hours') }}</p>
                                                </div>
                                            </div>
                                        </div>
                                    @else
                                            ...
                                            <div class="myJob-wrapper-single-balance-contents">
                                                <div
                                                    ...
                                                    <span class="myJob-wrapper-single-balance-icon hover-question">
                                                        <i class="fa-solid fa-question"></i>
                                                        <span
                                                            class="hover-active-content">{{ __('The approximate budget indicates the expected payment for this order. This amount may vary depending on the rate and the estimated working hours.') }}</span>
                                                    </span>
                                                </div>
                                                <p class="myJob-wrapper-single-balance-para">
                                                    {{ __('Approximate  Budget') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @else
                        ...
                        <div class="myOrder-single bg-white  padding-20 radius-10">
                            ...
                            @php
                                $check_order_has_report_by_client = \App\Models\Report::where(
                                    'client_id',
                                        ...
                                        <div class="left-part">
                                            <div class="inf-img">
                                                @if ($order_details->freelancer?->image)
                                                    <img src="{{ asset('assets/uploads/profile/' . $order_details->freelancer?->image) }}"
                                                        alt="{{ __('profile img') }}">
                                                @else
                                                    <img src="{{ asset('assets/static/img/author/author.jpg') }}"
                                                        alt="{{ __('profile img') }}">
                                                @endif
                                                <x-status.user-online-offline-check :userID="$order_details->freelancer?->id" />
                                            </div>
                                        </div>
                                        <div class="right-part">
                                            <div class="right-top d-flex gap-4">
                                                <div class="name lg-font fw_semibold black_text">
                                                    {{ $order_details->freelancer?->full_name }}
                                                    @if ($order_details?->freelancer->user_verified_status == 1)
                                            ...
                                            <div class="location-wraper ">
                                                ...
                                                <span class="location sm-font">
                                                    @if ($order_details->freelancer?->user_state?->state != null)
                                                        {{ optional($order_details->freelancer?->user_state)->state }},
                                                    @endif
                                                    {{ optional($order_details->freelancer?->user_country)->country }}
                                                </span>
                                            </div>
