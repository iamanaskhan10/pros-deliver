    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5 heading_title_for_promotion_modal" id="exampleModalLabel">
                    {{ __('Promotion Project') }}</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('influencer.package.buy') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                @php
                    $all_package = \Modules\PromoteInfluencer\Entities\ProjectPromoteSettings::select(
                        'id',
                        'title',
                        'budget',
                        'duration',
                ...
                <div class="modal-body">
                    <div class="alert alert-warning mb-5" role="alert">
                        <p class="warning_for_promotion_modal">
                    ...
                    <div class="single-input mb-3">
                        ...
                        <select id="get_package_budget" name="package_id" class="form-control get_package_budget">
                            ...
                            @foreach ($all_package as $package)
                                <option value="{{ $package->id }}" data-budget="{{ $package->budget }}">
                                    {{ $package->title }}
                                    (<strong>{{ __('Price:') }}</strong>{{ float_amount_with_currency_symbol($package->budget) }}/<strong>{{ __('Duration:') }}</strong>{{ $package->duration }}
                                    {{ __('days') }})</option>
                            @endforeach
                        </select>
                    </div>
                    <label class="checkbox-label" for="choose">
                        @if (Auth::check() && Auth::user()->user_wallet?->balance > 0)
                            ...
                            <span class="wallet-balance mt-2 d-block">{{ __('Wallet Balance:') }}
                                <strong
                                    class="main-balance">{{ float_amount_with_currency_symbol(Auth::user()->user_wallet?->balance) }}</strong>
                            </span>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary mt-4 pr-4 pl-4 confirm_promote_project">{{ __('Promote Now') }}
                        <span id="promote_project_load_spinner"></span></button>
                </div>
            </form>
        </div>
    </div>
