<div class="navbar-right-content show-nav-content">
    <div class="single-right-content">
        <div class="navbar-right-flex">
                        ...
                        <div class="header-global-search-select">
                            <select id="Select_project_or_job_for_search">
                                ...
                                @if (get_static_option('job_enable_disable') != 'disable')
                                    <option value="job">{{ __('Campaign') }}</option>
                                @endif
                                <option value="talent">{{ __('Influencer') }}</option>
                            </select>
                        </div>
                    </div>
            ...
            <div class="navbar-right-item position-relative">
                <a href="{{ route('user.login') }}" class="inf-cmn-btn style2 inf-black-btn-outline">
                    {{ __('Login') }}
                </a>
            </div>
            <div>
                <a href="{{ route('user.register') }}" class="inf-cmn-btn style2 inf-primary-btn">
                    {{ __('Join Now') }}
                </a>
            </div>
        </div>
