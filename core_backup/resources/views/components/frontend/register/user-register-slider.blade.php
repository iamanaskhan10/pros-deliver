<div class="login-right-item">
    <div class="login-right-shapes">
        <div class="login-right-thumb">
            @if(empty(get_static_option('register_page_sidebar_image')))
                <img src="{{ asset('assets/static/single-page/fr_1.png') }}" alt="{{ __('loginImg') }}">
            @else
    ...
    <div class="login-right-contents text-white">
        ...
        <p class="login-right-contents-para">{{ get_static_option('register_page_sidebar_description') ?? __('Once register you will see the magic of Influencer marketplace.') }}</p>
    </div>
</div>
