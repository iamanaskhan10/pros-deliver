    ...
    public function buy_package_user_email_settings(Request $request)
    {
        if ($request->isMethod('post')) {
            $request->validate([
                ...
                'user_promote_package_purchase_message' => 'required|min:10|max:100000',
            ]);
            $fields = [
        ...
        return view('PromoteInfluencer::backend.email.user-promote-package-email');
    }
    ...
    public function buy_package_admin_email_settings(Request $request)
    {
        if ($request->isMethod('post')) {
            $request->validate([
                ...
                'user_promote_package_purchase_message_admin' => 'required|min:10|max:100000',
            ]);
            $fields = [
        ...
        return view('PromoteInfluencer::backend.email.user-promote-package-email-to-admin');
    }
    ...
    public function buy_package_manual_payment_complete_email_settings(Request $request)
    {
        if ($request->isMethod('post')) {
            $request->validate([
                ...
                'user_promote_package_manual_payment_complete_message' => 'required|min:10|max:100000',
            ]);
            $fields = [
        ...
        return view('PromoteInfluencer::backend.email.user-promote-package-manual-payment-complete-email');
    }
}
