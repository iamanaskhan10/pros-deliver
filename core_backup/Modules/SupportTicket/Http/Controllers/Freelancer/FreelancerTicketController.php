...
class FreelancerTicketController extends Controller
{
    ...
    public function ticket(Request $request)
    {
        ...
        return view('supportticket::influencer.tickets',compact('tickets','departments'));
    }
    ...
    public function paginate(Request $request)
    {
       if($request->ajax()){
           ...
           return $tickets->total() >= 1
               ? view('supportticket::influencer.search-result', compact('tickets'))->render()
    ...
    public function ticket_details(Request $request, $id){
        $ticket_details = Ticket::with(['freelancer','message.admin_sender','get_ticket_latest_message'])
            ->where('id',$id)
        ...
        return !empty($ticket_details) ? view('supportticket::influencer.details',compact('ticket_details')) : back();
    }
}
