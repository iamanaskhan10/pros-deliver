...
@section('site_title')
    {{ $ticket_details->title ?? __('Support Tickets') }}
@endsection
@section('style')
    <style>
        .text_style_manege {
                                ...
                                <div class="supportTicket-single radius-10">
                                    <div class="supportTicket-single-item">
                                        <div class="supportTicket-single-flex">
                                            <div class="supportTicket-single-content">
                                                <div
                                                    class="supportTicket-single-content-header d-flex align-items-center gap-3">
                                                    <span
                                                        class="supportTicket-single-content-id">#{{ $ticket_details->id }}</span>
                                                    <div class="supportTicket-single-content-btn gap-2 flex-btn">
                                                        @if ($ticket_details->status == 'close')
                                                            <a href="javascript:void(0)" class="inf-status-badge danger">
                                                                {{ __('Closed') }}
                                                            </a>
                                                        @else
                                                            <a href="javascript:void(0)" class="inf-status-badge success">
                                                                {{ __('Open') }}
                                                            </a>
                                                        @endif
                                                        <a href="javascript:void(0)" class="inf-status-badge danger">
                                                ...
                                                <h4 class="supportTicket-single-content-title mt-2">
                                                    {{ $ticket_details->title }}</h4>
                                            </div>
                                            <span class="supportTicket-single-content-time">{{ __('Last update') }}
                                                ...
                                                <div class="supportTicket-single-chat">
                                                    <div class="supportTicket-single-chat-flex">
                                                        <div class="supportTicket-single-chat-thumb">
                                                            <img 
                                                                src="{{ get_attachment_image_by_id($message->admin_sender_or_default->image ?? '', 'thumb', true)['img_url'] }}"
                                                                alt="{{ __('admin') }}">
                                                        </div>
                                                        <div class="supportTicket-single-chat-contents">
                                                            <div class="supportTicket-single-chat-box">
                                                                <p
                                                                    class="supportTicket-single-chat-message text_style_manege">
                                                                    {{ $message->message }}
                                                                </p>
                                                            ...
                                                            <p class="supportTicket-single-chat-time mt-2">
                                            ...
                                            @else
                                                <div class="supportTicket-single-chat reply">
                                                    <div class="supportTicket-single-chat-flex">
                                                        <div class="supportTicket-single-chat-thumb">
                                                            @if (cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                                                <img src="{{ render_frontend_cloud_image_if_module_exists('profile/' . $ticket_details?->freelancer?->image, load_from: $ticket_details?->freelancer?->load_from ?? '') }}"
                                                                    alt="{{ __('freelancer') }}">
                                                            @else
                                                                <img src="{{ asset('assets/uploads/profile/' . $ticket_details->freelancer?->image) }}"
                                                                    alt="{{ __('freelancer') }}">
                                                            @endif
                                                        </div>
                                                        <div class="supportTicket-single-chat-contents">
                                    ...
                                    <div class="supportTicket-single-item">
                                        <div class="supportTicket-single-chat-replyForm">
                                            <x-validation.error />
                                            <form action="{{ route('influencer.ticket.details', $ticket_details->id) }}"
                                                ...
                                                <div
                                                    class="supportTicket-single-chat-replyForm-submit flex-between align-items-center mt-3">
                                                    <div>
                                                        <div class="supportTicket_single__attachment mt-3">
                                                            <input type="file" name="attachment" id="attachment">
                                                        </div>
                                                        <div class="supportTicket-single-chat-replyForm-input">
                                                            <label for="email_notify"><input type="checkbox"
                                                                    name="email_notify" id="email_notify">
                                                                {{ __('Email Notify') }}</label>
                                                        </div>
                                                    </div>
                                                    <div class="btn-wrapper d-flex flex-wrap gap-2">
                                                        <button type="submit"
                                                            class="btn-profile btn-bg-1 send_reply">{{ __('Send Reply') }}</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
...
@section('script')
    @include('supportticket::influencer.ticket-js')
