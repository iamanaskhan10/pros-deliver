...
[Checkout CinetPay Developer Documentation](https://docs.cinetpay.com/api/1.0-en/introduction/overview/)


### Detailed setup instructions [CinetPay.md](CinetPay.md).
...
```php
    $xendit = XgPaymentGateway::xendit();
...
```php
>>  Airwallex supported currency list  ['AUD', 'CNY', 'USD', 'HKD', 'EUR', 'GBP', 'CAD', 'SGD',
            ...
            'KES', 'PYG', 'ZMW', 'XAF']
````
#### airwallex ipn route example
````php
Route::post('/airwallex-ipn', [\App\Http\Controllers\PaymentLogController::class,'airwallex_ipn'] )->name('payment.airwallex.ipn');
````


#### charge_customer method example for airwallex
```php
    $airwallex = XgPaymentGateway::airwallex();
    $airwallex->setClientID('');
    $airwallex->setApiKey('');
    $airwallex->setCurrency("USD");
    $airwallex->setEnv(true); // this must be type of boolean , string will not work
    $airwallex->setExchangeRate(74); // if THB not set as currency
    $response =  $airwallex->charge_customer([
        'amount' => 10,
        'title' => 'this is test title',
        'description' => 'this is test description',
        'ipn_url' => route('post.ipn'), //post route
    ...
    return $response;
```
#### ipn_response method example for airwallex
...
```php
    ...
    $airwallex->setEnv(true); // this must be type of boolean , string will not work
    $airwallex->setExchangeRate(74); // if THB not set as currency
   \Log::info(print_r($airwallex->ipn_response(),true));
```

## AdyenPay Setup

[Checkout AdyenPay Developer Documentation](https://docs.adyen.com/)

### Detailed setup instructions [AdyenPay.md](AdyenPay.md).
