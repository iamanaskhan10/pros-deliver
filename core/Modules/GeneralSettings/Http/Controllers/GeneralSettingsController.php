...
class GeneralSettingsController extends Controller
{
    ...
    {
        $item_license_status = get_static_option('item_license_status');
        $item_license_msg = get_static_option('item_license_msg');

        return view('generalsettings::license-settings' , compact('item_license_status', 'item_license_msg'));
    }
    ...
    public function update_version_check(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                "msg" => __("Could not connect to the update server. Please try again later."),
                "type" => "danger",
                "markup" => "<p class='text-danger'>" . __("Update server is unreachable. Error: ") . $e->getMessage() . "</p>"
            ]);
        }
