...
use Modules\PromoteInfluencer\Entities\ProjectPromoteSettings;
...
class ProjectPromoteController extends Controller
{
    public function promote_settings(Request $request)
    {
        ...
        return view('PromoteInfluencer::backend.project-promote.settings.all-settings', compact('all_settings'));
    }
    ...
    public function paginate_promote_settings(Request $request)
    {
        if ($request->ajax()) {
            $all_settings = ProjectPromoteSettings::latest()->paginate(10);
            return view('PromoteInfluencer::backend.project-promote.settings.search-result', compact('all_settings'))->render();
        }
    }
