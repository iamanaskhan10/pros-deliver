...
use Modules\Subscription\Services\InfluencerSubscriptionService;
...
class UserChatService
{
    ...
    public static function send(int $client_id, int $freelancer_id, ?string $message, int $messageFrom, $file = null, ?int $project_id = null, $type = null, $proposal_id = null, $interview_message = null, $responseType = 'html'): View|Factory|array|string|Application|null
    {
        ...
        // Skip processing if no message or file is provided
        if (empty($message) && empty($file)) {
            return $instance->sendResponse(null, $instance->liveChat, $messageFrom, $responseType);
        }

        // Deduct connect if this is the first message in this chat
        $hasMessages = LiveChatMessage::where('live_chat_id', $instance->liveChat->id)->exists();
        if (!$hasMessages) {
            InfluencerSubscriptionService::deductConnect($freelancer_id);
        }
        ...
        // Store livechat message
        $messageRecord = $instance->storeMessage($messageFrom);

        // Fire pusher event
        $instance->fireEvent($messageRecord, $instance->liveChat, $messageFrom);

        // Trigger notifications
        if ($messageFrom == 2) {
            event(new UserNotificationEvent(__('You have a new chat message'), $client_id));
        }
        if ($messageFrom == 1) {
            event(new UserNotificationEvent(__('You have a new chat message'), $freelancer_id));
        }

        return $instance->sendResponse($messageRecord, $instance->liveChat, $messageFrom, $responseType);
    }
    ...
    private function sendResponse($message, $livechat, $messageFrom, $responseType){
        ...
        if($messageFrom == 2){
            return view("chat::components.influencer.message", [
                "data" => $livechat,
                "message" => $message
            ])->render();
    ...
    public function fireEvent($message, $livechat, $messageFrom): void
    {
        if($messageFrom == 2){
            ...
            $bladeMessage = view("chat::components.influencer.message", [
                "data" => $livechat,
                "message" => $message
            ])->render();
...
//            $resize_full_image = Image::make($file)->resize(300, 300);
