...
class WithdrawController extends Controller
{
    public function withdraw_request(FreelancerHandleWithdrawRequest $request)
    {
        ...
        if (moduleExists('SecurityManage') && auth()->user()->freeze_withdraw === 'freeze') {
            return back()->with(
                toastr_warning(__('Your withdrawal ability is currently frozen. Please contact support.'))
            );
        }
    ...
    public function withdraw_history()
    {
        ...
        return view('wallet::influencer.withdraw.requests', compact('all_request'));
    }
    ...
    function pagination(Request $request)
    {
        if ($request->ajax()) {
            $all_request = WithdrawRequest::where('user_id', auth()->user()->id)->latest()->paginate(10);
            return view('wallet::influencer.withdraw.search-result', compact('all_request'))->render();
        }
    }
}
