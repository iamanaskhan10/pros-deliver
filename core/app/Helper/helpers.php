...
function load_google_fonts()
{
    ...
    // body fonts
    $body_font_family = get_static_option('body_font_family') ?? 'Urbanist';
    $heading_font_family = get_static_option('heading_font_family') ?? 'Urbanist';
    $section_font_family = get_static_option('section_font_family') ?? 'Urbanist';
...
function get_footer_copyright_text()
{
    ...
    $footer_copyright_text = str_replace(['{copy}', '{year}'], ['&copy;', date('Y')], $footer_copyright_text);
...
function google_captcha_check($token)
{
    ...
    curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query(['secret' => get_static_option('site_google_captcha_v3_secret_key'), 'response' => $token]));
...
function project_rating($project_id)
{
    ...
    if ($project_complete_order_count >= 1 && $avg_rating >= 1) {
        $string = '<div class="single-project-content-review">
                     ...
                     <a href="#/" class="single-project-orderCompleted"> '.$project_complete_order_count.' '.__('Sold').' </a>';
    } elseif ($project_complete_order_count >= 1) {
        $string = '<div class="single-project-content-review">
                        <span class="single-project-content-review-rating">'.__('No Review').'</span>
                   </div>
                    <a href="#/" class="single-project-orderCompleted text-end"> '.$project_complete_order_count.' '.__('Sold').' </a>';
    } elseif ($project_complete_order_count < 1 && $avg_rating < 1) {
        $string = '<div class="single-project-content-review">
                        <span class="single-project-content-review-rating">'.__('No Review').'</span>
                   </div>';
    }
...
function single_project_rating($project_id)
{
    $project_complete_orders = Order::select('id', 'identity', 'status')->where('identity', $project_id)->where('status', 3)->get();
    $project_complete_order_count = $project_complete_orders->count();
...
function project_details_rating($project_id)
{
    $project_complete_orders = Order::select('id', 'identity', 'status')
        ->where('identity', $project_id)
        ->where('status', 3)
        ->get();

    $rating_buckets = [5.0 => 0, 4.0 => 0, 3.0 => 0, 2.0 => 0, 1.0 => 0];
    $total_rating = 0;
    $total_reviews = 0;
    ...
    $progress_html = '';
    foreach ([5.0, 4.0, 3.0, 2.0, 1.0] as $label) {
        $count = $rating_buckets[$label];
        $percent = $total_reviews > 0 ? ($count / $total_reviews) * 100 : 0;
        $progress_html .= '
            <div class="progress-wraper">
                <div class="progress">
                    <div class="progress-bar" role="progressbar" style="width: '.round($percent).'%" aria-valuenow="'.round($percent).'" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
                <div class="progress_label">
                    <span class="rating-label">'.number_format($label, 1).'</span>
    ...
    $star_html = '';
    for ($i = 1; $i <= 5; $i++) {
        $star_html .= '<span class="star '.($i <= $filled_stars ? 'filled' : '').'"><i class="fas fa-star"></i></span>';
    }

    $html = '
    <div class="left-part">
        <div class="overall-rating ">'.$avg_rating.'</div>
        <div class="stars-container">'.$star_html.'</div>
        <div class="total-reviews">'.$total_reviews.' '.__('Reviews').'</div>
    </div>
    <div class="right-part">'.$progress_html.'</div>';

    return $html;
}

function freelancer_details_rating($freelancer_id)
{
    $freelancer_orders = Order::select('id', 'freelancer_id', 'status')
...
function freelancer_order_rating($freelancer_id, $header = null)
{
        ...
        if ($complete_orders_count >= 1 && $avg_rating >= 1) {
            $string = ' <span class="star">
                            <svg width="20" height="21" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M9.04894 4.0208C9.3483 3.09949 10.6517 3.09949 10.9511 4.0208L11.7961 6.62161C11.93 7.03364 12.3139 7.3126 12.7472 7.3126H15.4818C16.4505 7.3126 16.8533 8.55221 16.0696 9.12161L13.8572 10.729C13.5067 10.9836 13.3601 11.435 13.494 11.847L14.339 14.4479C14.6384 15.3692 13.5839 16.1353 12.8002 15.5659L10.5878 13.9585C10.2373 13.7039 9.7627 13.7039 9.41222 13.9585L7.19983 15.5659C6.41612 16.1353 5.36164 15.3692 5.66099 14.4479L6.50604 11.847C6.63992 11.435 6.49326 10.9836 6.14277 10.729L3.93039 9.12162C3.14668 8.55221 3.54945 7.3126 4.51818 7.3126H7.25283C7.68606 7.3126 8.07001 7.03364 8.20389 6.62161L9.04894 4.0208Z" fill="#F0AD4E"></path>
                            </svg>
                        </span>
                        <span class="deep_black_text">'.round($avg_rating, 1).'</span>
                        <span>('.$rating_count.')</span>';
        }
...
function freelancer_rating_for_profile_details_page($freelancer_id)
{
    ...
    if ($complete_orders_count >= 1 && $rating_count >= 1) {
        ...
        $string = '<span class="inf-badge beige_badge">
                        <i class="fas fa-star"></i>
                        <span class="black_text"> '.round($avg_rating, 1.).' </span>
                        <span> ('.$rating_count.')</span>
                    </span>';
    } else {
        $string = '';
...
function freelancer_feedback_rating($freelancer_id)
{
    $feedbacks = \App\Models\Feedback::where('user_id', $freelancer_id)
        ->where('status', 1)
        ->get();
    ...
    $stars_html = str_repeat('<i class="fas fa-star"></i>', $full_stars);

    if ($half_star) {
        $stars_html .= '<i class="fas fa-star-half-alt"></i>';
    }

    $stars_html .= str_repeat('<i class="far fa-star"></i>', $empty_stars);

    $html = <<<HTML
    <div class="d-flex align-items-center gap-1 rating-wraper">
        <span>($average_rating)</span>
        <span class="golden_text sm-font">$stars_html</span>
    </div>
HTML;
...
if (! function_exists('truncateHtml')) {
    function truncateHtml($html, $limit = 150, $end = '...')
    {
...
function is_influencer_unlocked($influencer_id, $client_id = null)
{
    if (moduleExists('Credit')) {
        return \Modules\Credit\App\Services\CreditLogicService::isUnlocked($influencer_id, $client_id);
    }
    return true; // Default to unlocked if module is missing
}
