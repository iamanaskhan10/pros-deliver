...
class UserManageController extends Controller
{
    ...
    public function influencer_contact_info_settings(Request $request)
    {
        ...
        if ($request->isMethod('get')) {
            return view('backend.pages.user.influencer-contact-info-settings');
        }
    ...
    {
        $user_details = User::select(['id','first_name','last_name','email','username','user_type','image','hourly_rate','country_id','state_id','city_id',])->where('id',$request->user_id)->first();
    ...
    {
        $user = User::select(['id','apple_id'])->where('id',$id)->first();
        $user->apple_id == 'Yes' ? $status='No' : $status='Yes';
        User::where('id',$id)->update(['apple_id'=>$status]);
        return $status == 'Yes' ? back()->with(toastr_success(__('User Successfully Featured'))) : back()->with(toastr_success(__('User Successfully Remove From Featured')));
    }
    ...
    public function verify_user_phone($id)
    {
        $user = User::select(['phone_otp_verify','email','first_name','last_name'])->where('id',$id)->first();
        User::where('id',$id)->update([ 'phone_otp_verify' => 1]);
        try {
            $message = get_static_option('user_email_verified_message') ?? __('Your phone number successfully verified.');
            $message = str_replace(["@name"],[$user->first_name.' '.$user->last_name], $message);
            Mail::to($user->email)->send(new BasicMail([
                'subject' => get_static_option('user_email_verified_subject') ?? __('Disable 2FA Email'),
                'message' => $message
        ...
        return redirect()->back()->with(toastr_success(__('Phone Number Successfully Verified')));
    }
