...
class LanguageController extends Controller
{
    ...
    public function delete_language(Request $request, $id)
    {
        ...
        $backend_lang_file_path = resource_path('lang/') . $lang->slug . '.json';
        if (file_exists($backend_lang_file_path) && !is_dir($backend_lang_file_path)) {
            @unlink($backend_lang_file_path);
        }
        
        $lang->delete();
        return redirect()->back()->with(toastr_success(__('Language Delete Success')));
    }
