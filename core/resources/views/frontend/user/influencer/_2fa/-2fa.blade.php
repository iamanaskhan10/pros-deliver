                        ...
                        <div class="profile-settings-wrapper">
                            <div class="single-profile-settings">
                                ...
                                @if($user->google_2fa_secret == null)
                                    <form method="POST" action="{{ route('influencer._2fa.enable.disable') }}">
                                        @csrf
                                        <input type="hidden" name="form_type" value="generate_secret_key">
                                        <div class="btn-wrapper margin-top-50">
                                            <x-btn.submit :title="__('Generate Secret Key to Enable 2FAA')" :class="'btn-profile btn-bg-1'" />
                                        </div>
                                    </form>
                                ...
                                @elseif($user->google_2fa_enable_disable_disable == 1)
                                    ...
                                    <form method="POST" action="{{ route('influencer._2fa.enable.disable') }}">
                                        ...
                                        <div class="btn-wrapper profile-border-top flex-btn justify-content-end">
                                            <x-btn.submit :title="__('Disable 2FA')" :class="'btn-profile btn-bg-1 check_disable_2fa'" />
                                        </div>
                                    </form>
                                @endif
...
@section('script')
    ...
    @include('frontend.user.influencer._2fa.-2fa-js')
@endsection
