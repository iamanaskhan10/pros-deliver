...
@section('content')
    <div class="dashboard__body">
        <div class="myOrder_single bg-white padding-20 radius-10">
            <div class="row g-4">
                <div class="col-md-12">
                    <x-notice.general-notice :description="__('Notice: If a completed order is canceled by the admin, the order amount will be credited to the client wallet and deducted from the influencer wallet.')" :description1="__('Notice: If an order is in the queue and funded, an admin has the authority to decline it. If an admin decline a queued order, the order amount will be credited back to the client wallet.')" />
                </div>
                    ...
                    <div class="contract_single__balance hover-question">
                        <div class="contract_single__balance-flex flex-between">
                            ...
                            <span class="contract_single__balance-icon">
                                ...
                                <small class="hover-active-content">{{ __('Total paid means how much amount influencer get for this order.') }}</small>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                @else
                    ...
                    <div class="contract_single__balance hover-question">
                        <div class="contract_single__balance-flex flex-between">
                            ...
                            <span class="contract_single__balance-icon">
                                <i class="fa-solid fa-question"></i>
                                <small class="hover-active-content">{{ __('Pending amount means how much amount influencer will get after complete this order.') }}</small>
                            </span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="shop-close-content radius-10">
                        ...
                        <div class="single-shop-left bg-white radius-10 mt-5">
                            ...
                            <div class="btn-wrapper mt-4">
                                <a href="{{ route('influencer.profile.details',$order_details->freelancer?->username) }}" target="_blank" class="btn-profile btn-bg-1">{{ __('View Profile') }}</a>
                            </div>
                        </div>
                        ...
                        @if($order_details->rating)
                            @foreach($order_details->rating as $rating)
                                ...
                                @if($rating->sender_type == 2)
                                    <div class="single-shop-left bg-white radius-10 mt-5">
                                        <h6 class="user-details-manage-title">{{ __('Influencer Rating') }}
                                            <div class="single-project-content-review">
                                                <span class="single-project-content-review-icon"> <i class="fa-solid fa-star"></i> </span>
                                                <span class="single-project-content-review-rating">{{ $rating->rating }} </span>
                                            </div>
                                ...
                                <div class="myOrder_single__content">
                                    ...
                                    <h4 class="myOrder-single-content-title mt-2">
                                        @if($order_details->is_project_job == 'project')
                                            <a target="_blank" href="{{ route('shake.details', ['username' => $order_details?->project?->project_creator?->username, 'slug' => $order_details?->project->slug]) }}"> {{ $order_details?->project->title ?? '' }} </a>
                                ...
                                <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
                                    @if($order_details->description)
                                        <li class="nav-item" role="presentation">
                                            <h4>{{ __('Description & Requirements') }}</h4>
                                        </li>
                                    @endif
                                    @if($order_details?->order_submit_history->count() > 0)
                                        <li class="nav-item" role="presentation">
                                            <button class="btn" data-bs-toggle="pill" data-bs-target="#work_display" type="button" role="tab" aria-controls="pills-contact" aria-selected="false">{{ __('Works Submitted') }}</button>
                                        </li>
                                    @endif
                                </ul>
