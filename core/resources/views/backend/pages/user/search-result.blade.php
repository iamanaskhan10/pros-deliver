    ...
    @if($all_users->total() >=1)
        @foreach($all_users as $user)
            <tr>
                ...
                <td>{{ $user->apple_id === 'Yes' ? 'Yes' : 'No' }}</td>
                    ...
                    <ul class="dropdown-menu status_dropdown__list">
                        ...
                        <li class="status_dropdown__item">
                            <x-popup.delete-popup :title="__('Delete User')" :url="route('admin.user.delete',$user->id)"/>
                                ...
                                @if($user->phone_otp_verify == 0)
                                    <li class="status_dropdown__item">
                                        <x-status.table.phone-verify :title="__('Verify User Phone')" :url="route('admin.user.verify.phone',$user->id)"/>
                                    </li>
                                @endif
                        @endcan
                        ...
                        @endcan
                        @if($user->apple_id === 'Yes')
                            <li class="status_dropdown__item">
                                <x-status.table.status-change :title="__('Unfeatured')" :url="route('admin.user.make.featured',$user->id)"/>
                            </li>
                        @else
                            <li class="status_dropdown__item">
                                <x-status.table.status-change :title="__('Make Featured')" :url="route('admin.user.make.featured',$user->id)"/>
                            </li>
                        @endif
                        @can('user-account-status-change')
                        <li class="status_dropdown__item">
                        ...
                        @if(moduleExists('SecurityManage'))
                            ...
                            <x-status.table.status-change :title="__($is_withdrawal_freeze)" :url="route('admin.influencer.withdrawal.freeze',$user->id)"/>
                            <x-status.table.status-change :title="__($is_project_freeze)" :url="route('admin.influencer.project.freeze',$user->id)"/>
