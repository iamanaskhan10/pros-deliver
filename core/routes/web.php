...
Route::group(['middleware' => ['globalVariable', 'maintains_mode', 'setlang']], function () {
    ...
    Route::controller(RegisterController::class)->group(function () {
        ...
        Route::post('phone-number-availability', 'phoneNumberAvailability')->name('user.name.availability');
        ...
        Route::post( 'phone-otp-verify', 'phoneOtpVerify')->name('phone.otp.verify')->middleware('auth:web');
        Route::get('resend-verify-code-again', 'resendCode')->name('resend.verify.code')->middleware('auth:web');
        Route::get('resend-phone-otp-again', 'resendPhoneOtpCode')->name('resend.phone.otp.verify.code')->middleware('auth:web');
    });
    ...
    Route::controller(ProfileDetailsController::class)->group(function () {
        Route::get('freelancer/profile-details/{username}', 'profile_details')->name('influencer.profile.details');
        Route::get('freelancer/profile-reviews/{username}', 'profile_reviews')->name('influencer.profile.reviews');
        Route::post('freelancer/portfolio-details/display', 'portfolio_details')->name('influencer.portfolio.details');
        Route::match(['get','post'], 'toggle-earning', 'toggleEarning')->name('freelancer.toggle.earning')->middleware('auth:web');
    });
    ...
    Route::group(['middleware' => 'preventjoburl'], function () {
        ...
        Route::controller(FrontendJobsController::class)->group(function () {
            Route::get('campaigns/all', 'jobs')->name('jobs.all');
            Route::get('campaigns/all/filter', 'jobs_filter')->name('jobs.filter');
            Route::get('campaigns/all/pagination', 'pagination')->name('jobs.pagination');
            Route::get('campaigns/filter/reset', 'reset')->name('jobs.filter.reset');
        });
        // category campaigns
        Route::controller(CategoryJobController::class)->group(function () {
            Route::get('campaigns/categories/pagination', 'pagination')->name('category.jobs.pagination');
            Route::get('campaigns/categories/filter', 'category_jobs_filter')->name('category.jobs.filter');
            Route::get('campaigns/categories/filter/reset', 'reset')->name('category.jobs.filter.reset');
            Route::get('campaigns/categories/{slug}', 'category_jobs')->name('category.jobs');
        });
        // subcategory campaigns
        Route::controller(SubcategoryJobController::class)->group(function () {
            Route::get('campaigns/subcategories/pagination', 'pagination')->name('subcategory.jobs.pagination');
            Route::get('campaigns/subcategories/filter', 'subcategory_jobs_filter')->name('subcategory.jobs.filter');
            Route::get('campaigns/subcategories/filter/reset', 'reset')->name('subcategory.jobs.filter.reset');
            Route::get('campaigns/subcategories/{slug}', 'subcategory_jobs')->name('subcategory.jobs');
        });
        // skill campaigns
        Route::controller(SkillJobController::class)->group(function () {
            Route::get('campaigns/skill/pagination', 'pagination')->name('skill.jobs.pagination');
            Route::get('campaigns/skill/filter', 'skill_jobs_filter')->name('skill.jobs.filter');
            Route::get('campaigns/skill/filter/reset', 'reset')->name('skill.jobs.filter.reset');
            Route::get('campaigns/skill/{name?}', 'skill_jobs')->name('skill.jobs');
        });
    });
    ...
    Route::group(['middleware' => 'preventjoburl'], function () {
        ...
        Route::controller(JobDetailsController::class)->group(function () {
            Route::get('campaigns/{username}/{slug}', 'job_details')->name('job.details');
            Route::post('campaign/proposal/send/to-client', 'job_proposal_send')->name('job.proposal.send');
        });
    });
    ...
    Route::group(['middleware' => 'preventprojecturl'], function () {
        ...
        Route::controller(ProjectDetailsController::class)->group(function () {
            Route::get('projects/{username}/{slug}', 'project_details')->name('shake.details');
            Route::get('projects/reviews/{id}', 'get_project_reviews')->name('shake.reviews');
            Route::get('/project/review/load/more/data', 'load_more_review')->name('project.review.load.more');
        });
    });

    Route::get('project/reviews/{id}', [ProjectDetailsController::class, 'get_project_reviews'])->name('shake.reviews');
