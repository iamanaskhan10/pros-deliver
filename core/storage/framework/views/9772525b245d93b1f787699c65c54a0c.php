...
(function () {
    ...
    const GENERATE_URL = "<?php echo e(route('influencer.ai.proposal.generate')); ?>";
    const STATUS_BASE   = "<?php echo e(url('influencer/ai/proposal-status')); ?>";
    ...
    function resetButton() {
        ...
        $btnText.text("<?php echo e(__('✨ Generate with AI')); ?>");
    ...
    function setLoadingState() {
        ...
        $btnText.text("<?php echo e(__('Generating...')); ?>");
    ...
    function fillForm(data) {
        ...
        toastr_success_js("<?php echo e(__('AI proposal generated! Review and edit before sending.')); ?>");
    }
    ...
    function startPolling(uuid) {
        ...
        pollTimer = setInterval(function () {
            ...
            if (pollAttempts > MAX_POLL_ATTEMPTS) {
                stopPolling();
                resetButton();
                toastr_warning_js("<?php echo e(__('The AI is taking too long. Please try again.')); ?>");
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.status === 'done' && response.data) {
                        ...
                        // status === 'failed'
                        let msg = response.message || "<?php echo e(__('AI generation failed. Please write manually.')); ?>";
                ...
                error: function () {
                    stopPolling();
                    resetButton();
                    toastr_warning_js("<?php echo e(__('An error occurred while checking the AI status.')); ?>");
                }
            });
        }, POLL_INTERVAL_MS);
    }
    ...
    $(document).ready(function () {
        $(document).on('click', '#ai_generate_proposal_btn', function () {
            ...
            if (!jobId) {
                toastr_warning_js("<?php echo e(__('Job ID is missing. Cannot generate proposal.')); ?>");
                return;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 'processing' && response.uuid) {
                        ...
                        resetButton();
                        toastr_warning_js("<?php echo e(__('Unexpected response from AI. Please try again.')); ?>");
                    }
                },
                error: function (xhr) {
                    resetButton();
                    let msg = xhr.responseJSON?.message || "<?php echo e(__('AI request failed. Please try again.')); ?>";
                    toastr_warning_js(msg);
                }
            });
        });
...
<?php /**PATH /home/prosdeliver/public_html/core/resources/views/frontend/pages/job-details/ai-proposal-js.blade.php ENDPATH**/ ?>