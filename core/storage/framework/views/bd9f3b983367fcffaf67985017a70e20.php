...
(function ($) {
    ...
    $(document).on('click', '#btn_submit_ai', function () {
        ...
        if (description.length < 10) {
            toastr_warning_js('<?php echo e(__("Please enter at least 10 characters to describe your job.")); ?>');
            return;
        }
        ...
        $.ajax({
            url: '<?php echo e(route("client.job.generate.ai")); ?>',
            method: 'POST',
            data: {
            ...
            success: function (response) {
                if (response.status === 'success') {
                    ...
                    if (data.unmatched_skills && data.unmatched_skills.length > 0) {
                        var notice = '<?php echo e(__("AI suggested these skills not in our system:")); ?> <strong>' + data.unmatched_skills.join(', ') + '</strong>. <?php echo e(__("Please add them manually if needed.")); ?>';
                        $('#ai_unmatched_notice').html(notice).show();
                    }

                    toastr_success_js('<?php echo e(__("AI generated everything! Title, Description, Budget, and Skills are now filled. Check the next steps!")); ?>');
                    closeAiModal();

                } else {
                    toastr_error_js(response.message || '<?php echo e(__("AI generation failed. Please try again.")); ?>');
                }
            },
            error: function (xhr) {
                var msg = '<?php echo e(__("AI generation failed. Please fill the form manually.")); ?>';
                if (xhr.status === 422) {
                    var errors = xhr.responseJSON;
                    if (errors && errors.message) { msg = errors.message; }
                } else if (xhr.status === 429) {
                    msg = '<?php echo e(__("Too many requests. Please wait a moment and try again.")); ?>';
                }
                toastr_error_js(msg);
            },
            complete: function () {
...
<?php /**PATH /home/prosdeliver/public_html/core/resources/views/frontend/user/client/job/create/ai-job-generate-js.blade.php ENDPATH**/ ?>