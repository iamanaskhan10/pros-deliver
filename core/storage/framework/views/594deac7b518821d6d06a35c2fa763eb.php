    ...
    (function($){
        ...
        $(document).ready(function(){
            ...
            $(document).on('click','.user_details',function(){
                ...
                $('#edit_state').data('selected', state_id);
                $('#edit_city').data('selected', city_id);
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status == 'success') {
                            ...
                            $("#edit_state").html(all_options);
                            // reselect state if exists
                            let selected_state = $('#edit_state').data('selected');
                            if (selected_state) {
                                $("#edit_state").val(selected_state).trigger('change');
                            }

                            $(".state_info").html(all_state.length ? '' : '<span class="text-danger"><?php echo e(__("No state found for selected country!")); ?></span>');
                        }
                    }
                })
            })
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status == 'success') {
                            ...
                            $("#edit_city").html(all_options);

                            // reselect city if exists
                            let selected_city = $('#edit_city').data('selected');
                            if (selected_city) {
                                $("#edit_city").val(selected_city).trigger('change');
                            }

                            $(".city_info").html(all_city.length ? '' : '<span class="text-danger"><?php echo e(__("No city found for selected state!")); ?></span>');
                        }
                    }
                })
            })
            ...
            function countries(page){
                $.ajax({
                    url:"<?php echo e(route('admin.influencer.paginate.data').'?page='); ?>" + page,
                    success:function(res){
            ...
            $(document).on('keyup','#string_search',function(){
                ...
                $.ajax({
                    url:"<?php echo e(route('admin.influencer.search')); ?>",
                    method:'GET',
...
<?php /**PATH /home/prosdeliver/public_html/core/resources/views/backend/pages/user/user-js.blade.php ENDPATH**/ ?>