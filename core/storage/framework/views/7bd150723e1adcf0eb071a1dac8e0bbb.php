<script>
    (function($) {
        ...
        $(document).ready(function() {
            ...
            var notFoundHtml = <?php echo json_encode(view('components.frontend.not-found')->render(), 15, 512) ?>;
            ...
            $(document).on('click', '#job_search_by_text', function() {
                if ($('#job_search_string').val() == '') {
                    ...
                    $.ajax({
                        url: "<?php echo e(route('jobs.filter')); ?>",
                        method: 'GET',
                        data: {
                            country: country,
                            category: category,
            ...
            $(document).on('click', '#set_price_range', function(e) {
                ...
                if (!min_price_raw || !max_price_raw) {
                    toastr_warning_js("<?php echo e(__('Please select both price fields')); ?>");
                    return;
                }

                if (!isNaN(min_price) && !isNaN(max_price) && max_price < min_price) {
                    toastr_warning_js(
                        "<?php echo e(__('Maximum price must be greater than or equal to minimum price')); ?>"
                    );
                    return;
                }
            ...
            function jobs(page = 1) {
                ...
                $.ajax({
                    url: "<?php echo e(route('jobs.pagination') . '?page='); ?>" + page,
                    method: 'GET',
                    data: {
                        country: country,
                        category: category,
            ...
            $(document).on('click', '#campaign_filter_reset', function(e) {
                ...
                $.ajax({
                    url: "<?php echo e(route('jobs.filter.reset')); ?>",
                    method: 'GET',
                    success: function(res) {
                        if (res.status == 'nothing') {
                            $('.search_job_result').html(notFoundHtml);
...
<?php /**PATH /home/prosdeliver/public_html/core/resources/views/frontend/pages/jobs/jobs-filter-js.blade.php ENDPATH**/ ?>